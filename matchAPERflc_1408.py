...
def matchAPERflc(targname,dir='./',matchtol=3):

    puN = np.genfromtxt(dir+targname+'_allMatchedZPTed_pu.dat',names=True)
    pu = np.genfromtxt(dir+targname+'_allMatchedZPTed_pu.dat')

    psfN = np.genfromtxt(dir + targname + "_APER2flcTrans_1408.dat",names=True)
    psf = np.genfromtxt(dir + targname + "_APER2flcTrans_1408.dat")
    ...
    header1 += ' id_APERmatch '
    header2 = s0.join(col_ap)
    header2 += ' id_FLCmatch'
    #
    header = header1 + header2

    np.savetxt(dir+targname+'_APER2flcMatch_1408.dat',outArr,header=header)
