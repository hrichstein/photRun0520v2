...
cat = np.genfromtxt(dir+'jdan21l8q_HOROLOGIUM-I_F814W_t_2905rdrc1000.dat')
...
while (nF):
    matchrows = cat[(abs(master[row][x] - cat[:,0]) <= matchtol) & (abs(master[row][y] - cat[:,1]) <= matchtol)]

    if (len(matchrows) == 1):
      matchids[row][0] = matchrows[0][2]
      ...
      row = row + 1
         ...
         for dd in range(len(matchrows)):
             distDiff[dd] = np.sqrt( (master[row][x] - matchrows[dd][0])**2 +  (master[row][y] - matchrows[dd][1])**2)
         small = np.argmin(distDiff)
         matchids[row][0] = matchrows[small][2]
         row += 1
...
#
np.savetxt(dir+'drcFLCidx_lw.dat',master,header=header)
