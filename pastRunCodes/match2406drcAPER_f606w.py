...
while (nF):
    matchrows = cat[(abs(master[row][x] - cat[:,xt_v]) <= matchtol) & (abs(master[row][y] - cat[:,yt_v]) <= matchtol)]
    ...
    if (len(matchrows) == 1):
      ...
      row = row + 1
         ...
         for dd in range(len(matchrows)):
             distDiff[dd] = np.sqrt( (master[row][x] - matchrows[dd][xt_v])**2 +  (master[row][y] - matchrows[dd][yt_v])**2)
...
header= 'x y magr_606 id_cat'


#
np.savetxt(dir+'drcAPER_idx_2406_f606w.dat',master,header=header)
