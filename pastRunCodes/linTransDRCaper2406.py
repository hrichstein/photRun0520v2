...
def linTrans(targname,filt,dir='catRawMags1305/catDir/'):
    ...
    outname = "drc2APER.dat"

    new_match, new_all = test_linear(match[:,0],match[:,1], master[:,0], master[:,1], weights, weights, all[:,x_i],all[:,y_i])
...
def addTranscols(targname,filt,dir='catRawMags1305/catDir/'):
    ...
    cat = drc_file
#
    transCat = np.loadtxt(dir+"drc2APER.dat")
#
...
#
    ...
    np.savetxt(dir+'drc2APER_f814w_t.dat',cat,header=header)
#
#
    return None
