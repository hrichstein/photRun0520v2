...
def getRefPSFpu(targname,matchtol=3,dir='photUtils0820/'):
    ...
    psfN = np.genfromtxt(dir2+'matchedFLCpsf2506_all.dat',names=True)
    psf_cat = np.genfromtxt(dir2+'matchedFLCpsf2506_all.dat')
    ...
    psf_cat = np.hstack((psf_cat,id_psf))
    apNames = np.array(psfN.dtype.names)
    ...
    xA_tru = np.int(np.where(apNames=='xPSF')[0])
    yA_tru = np.int(np.where(apNames=='yPSF')[0])

    p50 = np.argsort(pUN['magr_f606w'])[:50]
    fp_50 = pU_cat[p50]

    a50 = np.argsort(psfN['m606cPSF'])[:50]
    fa_50 = psf_cat[a50]
    ...
    header = 'xPU_f606w yPU_f606w idPU idPSF xPSF yPSF'

    outName = dir+'drcPSFref_'+targname
    np.savetxt(outName+'_pU.dat',outArr,header=header)

    fig, ax = plt.subplots(figsize=(6,6))

    ax.scatter(outArr[:,xP],outArr[:,yP],label='PhotU',s= 50)
    ax.scatter(regAP[:,xA],regAP[:,yA],label='PSF',s=20)
...
getRefPSFpu('HOROLOGIUM-I',matchtol=1,dir='photUtils0820/')
