...
def main():
    ...
    pu_file = '../drcPhot02Nov/sePU10Nov/' + 'puWtrans.dat'
    save_dir = '../drcPhot02Nov/sePU10Nov/'
    tname='HOROLOGIUM-I'
    se_file = '../catMatchFLCdrc18Oct/seDRCs/' + tname + '_sfErr.dat'
    matchTransSEpu(tname,seFilename=se_file,puFilename=pu_file,
                   saveDir=save_dir)
...
def matchTransSEpu(targname,seFilename='None.dat',puFilename='None.dat',
    ...
    xPU = np.int(np.where(puNames=='xSEf814w')[0])
    yPU = np.int(np.where(puNames=='ySEf814w')[0])
