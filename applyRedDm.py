...
def applyRedDm(targname,dir='./'):
    ...
    zptFile = np.genfromtxt(dir+targname+'_allMatchedZPTed_pu.dat',names=True)
    zptCat = np.genfromtxt(dir+targname+'_allMatchedZPTed_pu.dat')
    colZs = np.array(zptFile.dtype.names)

    vMag = zptFile['magZPT_f606w']
    iMag = zptFile['magZPT_f814w']
