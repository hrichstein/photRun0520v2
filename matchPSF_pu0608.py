...
def matchPSFPu(targname,dir='./',matchtol=3):

    puN = np.genfromtxt(dir+'HOROLOGIUM-I_filtMatch_pU.dat',names=True)
    pu = np.genfromtxt(dir+'HOROLOGIUM-I_filtMatch_pU.dat')

    psfN = np.genfromtxt(dir+'HOROLOGIUM-I_PSFpuTrans_pU.dat',names=True)
    psf = np.genfromtxt(dir+'HOROLOGIUM-I_PSFpuTrans_pU.dat')
