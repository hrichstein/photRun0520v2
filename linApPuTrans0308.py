...
def linAperTrans(targname,dir='photUtils0820/'):

    # Going from F814W to F606W

    file = np.genfromtxt(dir+'drcAPERref_'+targname+'_pU.dat',names=True)
    fileCat = np.genfromtxt(dir+'drcAPERref_'+targname+'_pU.dat')
    colNs = np.array(file.dtype.names)

    dir2 = '/Volumes/Spare Data/Hannah_Data/mattia/rephotometryquestion/'
    all = np.genfromtxt(dir2+'HOROLOGIUM_CF.2.TOSEND.CAT',names=True)
    allCat = np.genfromtxt(dir2+'HOROLOGIUM_CF.2.TOSEND.CAT')
    ...
    xa_2p = np.int(np.where(colNs=='xAPER')[0])
    ya_2p = np.int(np.where(colNs=='yAPER')[0])
    ...
    xp = np.int(np.where(colNs=='xPU_f606w')[0])
    yp = np.int(np.where(colNs=='yPU_f606w')[0])
    ...
    new_match[:,0],new_match[:,1],master_arr[:,0], master_arr[:,1],label_1='Original in F814W',label_2='New in APER 2 PU',label_3='Original in PU',outname=outName+'_matchCheck')
...
linAperTrans('HOROLOGIUM-I',dir='photUtils0820/')
