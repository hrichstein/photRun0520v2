...
def getZPT(targname,filt,dir='./',sigTol=2.5,stdTol=0.1):
    ...
    file_idx = np.logical_and(np.logical_and(file['magrF']>=19.5,file['magrF']<=26.5),file['stdF']<=stdTol)
    file_g = file[file_idx]

    full_idx = np.logical_and(np.logical_and(full[magStr]>=19.5,full[magStr]<=26.5),full['stdF']<=stdTol)
    full = full[full_idx]
    fullStr = 'magrD'+fils
    # while kG:
    ...
    plt.savefig(saveDir+targname+'_'+filt+'ZPTline2.png',dpi=600,bbox_inches='tight')
...
def applyZPT(mag_corr,err_add,targname,filt,dir='./'):
    ...
    np.savetxt(saveDir+'magZPTedAll_'+filt+'_2.dat',outArr,header=header)
...
def doZPT(targname,filt,dir='./',sigTol=2.5,stdTol=0.05):
    ...
    mag_corr,err_add = getZPT(targname,filt,dir=dir,sigTol=sigTol,stdTol=stdTol)
