...
def whichIter(targname,filt='F606W',dir='./',matchtol=10):
    ...
    diag = open(dir+'iterDiags_all_nonZPT.dat','w')
    ...
    file_str = dir+filt+"_ALL_drcTrans_noZPT"+str(int(takeRun))+".dat"
...
def getMatch(targname,file,filt='F606W',dir='./',matchtol=2.5,stdTol=5):
    ...
    outName = dir+'flcDRCmatch_ALL_nonZPT'+filt
    ...
    outName = dir+'magDiffs_nonZPT'+filt
    ...
    np.savetxt(dir+targname+'_matchedFLC_DRC_onZPT_'+filt+'.dat',out2,header=header)
...
def getRef_i(targname,filt='F606W',dir='./',matchtol=5,stdTol=2.5,iter=0):
    ...
    if iter==0:
        ...
        flcN = np.genfromtxt(dir+filt+"_ALL_drcTrans_noZPT{}.dat".format(iter-1),names=True)
        flc = np.genfromtxt(dir+filt+"_ALL_drcTrans_noZPT{}.dat".format(iter-1))
    ...
    outName = dir+'flcDRCref_ALL_noZPT'+str(iter)
...
def linFLC2drc(targname,filt='F606W',dir='./',iter=0):
    ...
    file = np.genfromtxt(dir+'flcDRCref_ALL_noZPT'+str(iter)+'.dat',names=True)
    fileCat = np.genfromtxt(dir+'flcDRCref_ALL_noZPT'+str(iter)+'.dat')
    ...
    if iter==0:
        ...
        all = np.genfromtxt(dir+filt+"_ALL_drcTrans_noZPT"+str(iter-1)+'.dat',names=True)
        allCat = np.genfromtxt(dir+filt+"_ALL_drcTrans_noZPT"+str(iter-1)+'.dat')
    ...
    iterStr = str(iter)
    outName = dir+filt+"_ALL_drcTrans_noZPT"+iterStr
