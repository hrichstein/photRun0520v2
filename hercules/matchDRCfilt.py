...
def matchFiltDRC(targname,f6d,dir='./',matchtol=2,suffix='_pu.dat',setnum=0):
    ...
    f814wN = np.genfromtxt(dir + "drcFiltTrans_" + targname + str(setnum)
                           + ".dat",names=True)
    f814w = np.genfromtxt(dir + "drcFiltTrans_" + targname + str(setnum)
                          + ".dat")
    ...
    np.savetxt(dir + targname + '_matchedDRCfilt' + str(setnum) + '.dat',
               outArr, header=header)
